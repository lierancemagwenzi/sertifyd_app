...
class AdminCertItemWidget extends StatefulWidget {
  final bool expanded;
...
final Color color;
  ...
  final ValueChanged<String> Onnosignature;

  final ValueChanged<CertificationApplicationModel> onClick;


  AdminCertItemWidget({Key key, this.expanded, this.applicationModel, this.onCanceled,this.onClick,this.onSelected,this.onReschedule,this.scaffoldKey,this.Onnosignature,this.color}) : super(key: key);

  @override
  _AdminCertItemWidgetState createState() => _AdminCertItemWidgetState();
}

class _AdminCertItemWidgetState extends State<AdminCertItemWidget> {
  @override
    ...
    return  1==1?Padding(
      ...
      child: InkWell(
        ...
        onTap: (){
          ...
          if( widget.applicationModel.status.admin_next!=null&&widget.applicationModel.status.admin!=null){

              if(currentuser.value.signature!=null){
                widget.onClick(widget.applicationModel);

                // Navigator.of(context).pushNamed('/${widget.applicationModel.status.admin_next.toLowerCase()}', arguments: widget.applicationModel.id);

              }
              else{

                widget.Onnosignature("First add a signature under profile->settings->signature");
                // s_con.caffoldKey?.currentState?.showSnackBar(SnackBar(
...
SizedBox(width: 10.0,),
              ...
              Container(
                child: Wrap(
                  ...
                  children: <Widget>[
                    widget.applicationModel.status.admin_next!=null&&widget.applicationModel.status.admin!=null? FlatButton(
                      onPressed: () {
                        if(currentuser.value.signature!=null){
                             widget.onClick(widget.applicationModel);
                      ...
                      child: Wrap(
                        children: <Widget>[Text(widget.applicationModel.status.admin)],
                      ),
