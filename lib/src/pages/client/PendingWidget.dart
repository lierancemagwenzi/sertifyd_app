...
class PendingApplicationWidget extends StatefulWidget {

  ClientApplication  clientApplication;

  PendingApplicationWidget({Key key, this.clientApplication}) : super(key: key);

  @override
  _PendingApplicationWidgetState createState() => _PendingApplicationWidgetState();
}

class _PendingApplicationWidgetState extends StateMVC<PendingApplicationWidget> {

  PaymentController _con;

  _PendingApplicationWidgetState() : super(PaymentController()) {
    _con = controller;
  }
  ...
  void initState() {
    ...
    if(widget.clientApplication.message!=null){

      Future.delayed(const Duration(seconds: 1), () {

        _con.scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(widget.clientApplication.message??''),
        ));
...
//             ),
  ...
  Widget _submitButton() {
    return InkWell(
      onTap: (){
        _con.RequestForm(widget.clientApplication.id, context);
      },
      child: Container(
  ...
  Commisioner(){
    ...
    return  1==1?SizedBox(height: 0,):Padding(
      padding: const EdgeInsets.symmetric(horizontal:15.0),
